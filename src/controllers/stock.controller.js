...
exports.createStockEntry = async (req, res) => {
  try {
    const { product_id, entry_date, opening_quantity, remarks } = req.body

    if (!product_id || !entry_date || !opening_quantity) {
      return res.status(400).json({ message: "Missing required fields: product_id, entry_date, or opening_quantity." })
    }

    // Get latest entry to compute current balance
...
exports.getAllStockEntries = async (req, res) => {
  try {
    const entries = await stock_entries.findAll({
      include: [{ model: Product, as: 'product', attributes: ['name'] }], // 👈 use 'as'
      order: [['entry_date', 'DESC']]
    });
    res.json(entries);
  } catch (err) {
